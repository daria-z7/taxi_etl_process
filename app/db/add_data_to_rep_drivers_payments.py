...
def load_fio_drivers_card_amt():
    ...
    cur = conn_2.cursor()
    with cur as cur:
        cur.execute(""" SELECT fr.driver_pers_num, dd.last_name, dd.first_name, dd.middle_name, dd.card_num
                        , round((fr.price_amt - (fr.price_amt*.2 + 47.26*7*fr.distance_val/100 + 5*fr.distance_val)), 2) as amount
                        FROM fact_rides fr left join dim_drivers dd on fr.driver_pers_num = dd.personnel_num
                        WHERE DATE(ride_end_dt) = CURRENT_DATE - 15
                    """)
        query = cur.fetchall()
...
def add_data_to_rep_drivers_payments(df: list):
    if not df.empty:
        conn_2 = create_conn_2()
        cur = conn_2.cursor()
        report_dt = date.fromordinal(date.today().toordinal() - 1)
